...
function updateReelsUI() {
    ...
    if (progress <= 0) {
        ...
        if($(".rock").length == 0){
            ...
            // Block scrolling completely using capture phase
            ...
            window.addEventListener('wheel', stopEvent, {passive:false, capture:true});
            window.addEventListener('touchmove', stopEvent, {passive:false, capture:true});
            window.addEventListener('touchstart', stopEvent, {passive:false, capture:true});
