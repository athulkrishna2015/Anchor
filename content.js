...
function updateReelsUI() {
    ...
    if (progress <= 0) {
        ...
        if($(".rock").length == 0){
            ...
            // Block scrolling
            $("body").append('<div id="anchor-blocker" style="position:fixed;top:0;left:0;width:100%;height:100%;z-index:9999999999;background:transparent;"></div>');
            document.getElementById('anchor-blocker').addEventListener('wheel', function(e){e.preventDefault(); e.stopPropagation();}, {passive:false});
            document.getElementById('anchor-blocker').addEventListener('touchmove', function(e){e.preventDefault(); e.stopPropagation();}, {passive:false});
            document.getElementById('anchor-blocker').addEventListener('keydown', function(e){
                if(["ArrowDown","ArrowUp","Space","PageDown","PageUp"].indexOf(e.code) > -1) {
                    e.preventDefault();
                }
            }, {passive:false});
        }
